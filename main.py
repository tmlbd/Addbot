...
SECRET_KEY = os.environ.get("SECRET_KEY", "EARN_PRO_MASTER_FINAL_2025")
...
db = client['mega_earning_final_v50'] # New Final Version
...
if bot:
    ...
    def start_cmd(message):
        ...
        refers = user['ref_count'] if user else 0
        ...
        msg = (
            ...
            f"👥 মোট রেফার: `{refers}` জন\n\n"
            f"📢 **নোটিশ:** {config['notice']}\n\n"
            f"👇 এড দেখে আয় করতে নিচের বাটনে ক্লিক করুন।"
        )
...
<head>
    ...
    <style>
        ...
        .notice-bar { background: linear-gradient(90deg, #f59e0b, #d97706); color: #000; width: 100%; padding: 12px; font-weight: 700; font-size: 13px; text-align: center; position: sticky; top: 0; z-index: 100; }
        .container { width: 100%; max-width: 440px; padding: 20px; box-sizing: border-box; }
        .glass-card { background: var(--card); backdrop-filter: blur(15px); border-radius: 30px; padding: 25px; border: 1px solid rgba(255,255,255,0.1); box-shadow: 0 25px 50px rgba(0,0,0,0.5); text-align: center; }
        .avatar { width: 75px; height: 75px; border-radius: 50%; border: 3px solid var(--primary); margin-bottom: 10px; }
        ...
        .stat-box { background: rgba(255,255,255,0.03); padding: 12px; border-radius: 18px; border: 1px solid rgba(255,255,255,0.05); }
        .btn { width: 100%; padding: 16px; border: none; border-radius: 18px; font-size: 16px; font-weight: 700; cursor: pointer; transition: 0.3s; margin-bottom: 10px; color: #fff; }
        .btn-work { background: linear-gradient(to right, #10b981, #059669); }
        .btn-withdraw { background: #1e293b; }
        .btn-recharge { background: #8b5cf6; }
        .ref-box { background: rgba(0,0,0,0.3); padding: 12px; border-radius: 15px; font-size: 10px; color: #60a5fa; border: 1px solid #1e293b; margin-top: 15px; word-break: break-all; text-align: left; }
        ...
        .modal-content { background: #161e31; margin: 15% auto; padding: 25px; width: 85%; max-width: 380px; border-radius: 25px; border: 1px solid #334155; }
        input, select { width: 100%; padding: 12px; margin: 8px 0; border-radius: 10px; background: #0b0f1a; color: #fff; border: 1px solid #334155; box-sizing: border-box; }
    </style>
</head>
<body>
    ...
    <div class="container">
        <div class="glass-card">
            ...
            <p style="font-size:11px; color:#94a3b8;">ID: #{{ user.user_id }}</p>
            <div class="balance-card">
                <small style="opacity:0.8;">Total Balance</small>
                <div class="balance-amount">৳ <span id="bal">{{ "%.2f"|format(user.balance) }}</span></div>
            </div>
            <div class="stats-grid">
                <div class="stat-box"><small>Refers</small><div style="font-size:18px;">{{ user.ref_count }}</div></div>
                <div class="stat-box"><small>Ads Left</small><div id="daily_left" style="font-size:18px;">{{ config.daily_ad_limit - user.daily_views }}</div></div>
            </div>
            ...
            <p style="font-size:10px; color:#64748b; margin-top:10px;">Reset In: <span id="timer" style="color:#fff;">--:--:--</span></p>
        </div>
    </div>
    ...
    <div id="withdrawModal" class="modal">
        <div class="modal-content">
            ...
            <button class="btn" onclick="closeM('withdrawModal')" style="background:transparent;">Cancel</button>
        </div>
    </div>
    ...
    <div id="rechargeModal" class="modal">
        <div class="modal-content">
            ...
            <button class="btn" onclick="closeM('rechargeModal')" style="background:transparent;">Cancel</button>
        </div>
    </div>
        ...
        if (typeof window['show_'+zid] === 'function') {
            ...
            function runLoop() {
                if (adsDone < totalAds) {
                    ...
                    adsDone++;
                    if(adsDone < totalAds) {
                        document.getElementById('workBtn').innerText = "Ad " + (adsDone+1) + " Loading...";
...
<head>
    ...
    <style>
        ...
        input, textarea, select { width: 100%; padding: 12px; margin: 5px 0; border-radius: 10px; background: #0b0f1a; color: #fff; border: 1px solid #334155; box-sizing: border-box; }
...
<body>
    <h1 style="text-align:center;">👑 Admin Master Panel</h1>
    <div class="grid">
        <div class="card">
            ...
            <form method="POST" action="/admin/save_settings">
                ...
                <select name="ad_interval">
                    {% for i in range(1, 16) %}<option value="{{i}}" {% if config.ad_interval == i %}selected{% endif %}>{{i}} Sec</option>{% endfor %}
                </select>
                VPN Security: 
                <select name="vpn_on">
                    <option value="on" {% if config.vpn_on %}selected{% endif %}>ON (Locked)</option>
                    <option value="off" {% if not config.vpn_on %}selected{% endif %}>OFF (Unlocked)</option>
                </select>
        ...
        <div class="card">
            <h3>💰 Withdrawals/Recharge</h3>
            <table>
                <tr><th>User</th><th>Amt</th><th>Acc</th><th>Action</th></tr>
                {% for w in withdraws %}
                <tr><td>{{w.name}}<br><small>{{w.type}}</small></td><td>৳{{w.amount}}</td><td>{{w.account}}</td><td><a href="/admin/pay/{{w._id}}" style="color:red;">Paid</a></td></tr>
                {% endfor %}
            </table>
        </div>
    </div>
...
def home():
    ...
    if not user_id: return "<h1>Join via Telegram Bot!</h1>", 403
